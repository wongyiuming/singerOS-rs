...
use web_sys::{
    ...
    DynamicsCompressorNode, GainNode, MediaDeviceInfo, MediaDeviceKind, MediaRecorder, MediaStream,
...
pub async fn open(device_id: Option<String>, echo_cancellation: bool) -> Result<(), String> {
    ...
    Reflect::set(
        ...
        &JsValue::from_bool(echo_cancellation),
    )
    .map_err(js_error)?;
    ...
    let record_destination = context
        .create_media_stream_destination()
        .map_err(js_error)?;
    source
        .connect_with_audio_node(&record_gain)
        .map_err(js_error)?;
    record_gain
    ...
    limiter
        ...
        .map_err(js_error)?;
    analyser
        .connect_with_audio_node(&record_destination)
        .map_err(js_error)?;
    ...
    let compressor: DynamicsCompressorNode =
        context.create_dynamics_compressor().map_err(js_error)?;
    ...
    source
        .connect_with_audio_node(&highpass)
        .map_err(js_error)?;
    ...
    notch
        ...
        .map_err(js_error)?;
    compressor
        .connect_with_audio_node(&monitor_gain)
        .map_err(js_error)?;
    monitor_gain
    ...
    let afs_loop = Interval::new(250, move || {
        ...
        let Some((idx, peak)) = bins.iter().copied().enumerate().max_by_key(|(_, v)| *v) else {
            return;
        };
        if peak < 232 {
            return;
        ...
        if (170.0..=8000.0).contains(&hz) {
            n.frequency().set_value(hz);
            n.q().set_value(14.0);
        }
    });

    ENGINE.with(|slot| {
        *slot.borrow_mut() = Some(AudioEngine {
            context,
            stream,
            record_gain,
            monitor_gain,
...
pub fn set_record_gain(percent: u16) {
    ENGINE.with(|slot| {
        if let Some(e) = slot.borrow().as_ref() {
            e.record_gain
                .gain()
                .set_value((percent.min(300) as f32) / 100.0);
        }
    });
}
...
pub fn set_monitor_gain(percent: u16) {
    ENGINE.with(|slot| {
        if let Some(e) = slot.borrow().as_ref() {
            ...
            if e.monitor_enabled.get() {
                e.monitor_gain.gain().set_value(value);
            }
        }
    });
}
...
pub fn set_monitor_enabled(on: bool) {
    ENGINE.with(|slot| {
        if let Some(e) = slot.borrow().as_ref() {
            ...
            e.monitor_gain
                .gain()
                .set_value(if on { e.monitor_level.get() } else { 0.0 });
        }
    });
}
...
pub fn meter_percent() -> u16 {
    ENGINE.with(|slot| {
        ...
        let rms = (samples
            .iter()
            .map(|v| ((*v as f64 - 128.0) / 128.0).powi(2))
            .sum::<f64>()
...
pub async fn start_recording(
    ...
    let stream = ENGINE
        .with(|slot| {
            slot.borrow()
                .as_ref()
                .map(|e| e.record_destination.stream())
        })
        .ok_or("microphone is not open")?;
    let recorder = MediaRecorder::new_with_media_stream(&stream).map_err(js_error)?;
    let mime = if recorder.mime_type().is_empty() {
    ...
    if let Some(obj) = start_body.as_object_mut() {
        obj.insert("mime".into(), Value::String(mime));
    }
        ...
        spawn_local(async move {
            ...
            if let Some(obj) = body.as_object_mut() {
                obj.insert(
                    ...
                    json!(((Date::now() - started_ms) / 1000.0).max(0.0)),
                );
            }
    ...
    RECORDING.with(|slot| {
        *slot.borrow_mut() = Some(RecordingRuntime {
            recorder,
            _data: data_cb,
            _stop: stop_cb,
        })
